...
NULL
#' Fit the doubly-censored AFT model with linear regression model
#'
...
#'dcpenwls(L,R,T,delta,x,type="wlse")
#'dcpenwls(L,R,T,delta,x,type="scad")
#'dcpenwls(L,R,T,delta,x,type="wlse",wttype="Beran",hlimit=0.1)
#' }
#' @export
#'
#'
...
dcpenwls=function(L,R,T,delta,x,beta0=rep(1,ncol(x)),type="wlse",wttype="KM",hlimit=0.1,contx1.pos=1,contx2.pos=1,selection=FALSE,id=NULL,index=1,maxit=100,tol=1e-3){
  library(extRemes)
  library(MASS)
  library(tidyverse)
  library(survival)
  ...
  Betafunc = function(L,R,T,x,delta,ww,eta,lambda=NULL,type){
    ...
    if (type=="wlse") {
      ...
      beta=lassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$beta
      betas = lassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$betas
      lambda=lassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$lambda
      best_lambda=lassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$best_lambda
      cve=lassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$cve
      cvse=lassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$cve
    }else if(type=="alasso"){
      init=init[-1]
      beta=alassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$beta
      betas = alassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$betas
      lambda=alassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$lambda
      best_lambda=alassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$best_lambda
      cve=alassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$cve
      cvse=alassopen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$cve
    }else if(type=="elasticnet"){
      beta=elasticpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$beta
      betas = elasticpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$betas
      lambda=elasticpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$lambda
      best_lambda=elasticpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$best_lambda
      cve=elasticpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$cve
      cvse=elasticpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta)$cve
    }else if(type=="scad"){
      init=init[-1]
      beta=scadpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$beta
      betas = scadpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$betas
      lambda=scadpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$lambda
      best_lambda=scadpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$best_lambda
      cve=scadpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$cve
      cvse=scadpen(L=L,R=R,T=T,x=x,delta=delta,ww=ww,eta=eta,init=init)$cve
    }
    else if(type=="oracle"){
  ...
  lassopen=function(L,R,T,x,delta,ww,eta){
    p=ncol(x); nonzero_p=sum(beta0!=0)
  ...
  alassopen=function(L,R,T,x,delta,ww,eta,init){
    p=ncol(x); nonzero_p=sum(beta0!=0)
    X=(ww*eta*x); Y=(ww*eta*Y)
    
    # Fitting an alasso regression model.
    alasso_model <- ncvreg(X, Y, penalty = "lasso", penalty.factor = (1/abs(init)))
  ...
  elasticpen=function(L,R,T,x,delta,ww,eta){
    L = pmax(L,1e-8); R=pmax(R,1e-8); Y=ifelse(L<R, pmin(R,pmax(L,T)), pmin(R,T) );n=length(Y);
    p=ncol(x); nonzero_p=sum(beta0!=0)
    X=(ww*eta*x); Y=(ww*eta*Y)
    
    # Fitting an elasticnet regression model.
    elastic_net_model <- glmnet(X, Y, alpha = 0.5)  # alpha = 0.5: Elastic Net
    # Selecting the optimal lambda (Lambda) using cross-validation.
  ...
  scadpen=function(L,R,T,x,delta,ww,eta,init){
    L = pmax(L,1e-8); R=pmax(R,1e-8); Y=ifelse(L<R, pmin(R,pmax(L,T)), pmin(R,T) );n=length(Y);
    p=ncol(x); nonzero_p=sum(beta0!=0)
    X=(ww*eta*x); Y=(ww*eta*Y)
  ...
  Mmat=function(L,R,T,x,delta,beta,ww,eta,cluster,type){
    ...
    deltaR = ifelse(delta==2,1,0)
    if(sum(deltaL)==0){
      deltaL=ifelse(delta==0,0,1)
    }
    if(sum(deltaR)==0){
      deltaR=ifelse(delta==0,0,1)
    }
    ...
    for(i in 1:n){
      ...
      Dr=rbind(Dr,D1r)
      M4=(deltaR[i]*yind[i]*D1r*eta[i])/(sum(yind[i]*eta ) )
      M44=rbind(M44,M4)
      D1l=(colSums(ww*(1-yind)*xx*res*eta)/(n+1-denom))
      Dl=rbind(Dl,D1l)
      M5=(deltaL[i]*(1-yind[i])*D1l*eta[i])/(n+1-sum(yind[i]*eta ) )
      M55=rbind(M55,M5)
    }
    M2=deltaR*Dr
    M3=deltaL*Dl
  ...
  if(wttype=="Beran" & is.null(hlimit)==F){ww=Berfunc(L=L,R=R,T=T,delta=delta,x=x);}
  # p=ncol(x); init = beta = DCls(L=L,R=R,T=T,delta=delta,x=x,ww=ww,eta=eta)
  
  if(type=="wlse"&selection==TRUE){
    new_beta = Betafunc(L=L,R=R,T=T,delta=delta,x=x,ww=ww,eta=eta,type=type)$beta[-1]
  }else if(type=="wlse"){
    new_beta = Betafunc(L=L,R=R,T=T,delta=delta,x=x,ww=ww,eta=eta,type=type)$beta
    Afunc=A=Amat(type=type,x=x,ww=ww,eta=eta,cluster=cluster); 
    Mfunc=M=Mmat(L=L,R=R,T=T,x=x,delta=delta,beta=new_beta,ww=ww,eta=eta,cluster=cluster,type=type)
  ...
  betas = Betafunc(L=L,R=R,T=T,delta=delta,x=x,ww=ww,eta=eta,type=type)$betas
  lambda = Betafunc(L=L,R=R,T=T,delta=delta,x=x,ww=ww,eta=eta,type=type)$lambda
  best_lambda = Betafunc(L=L,R=R,T=T,delta=delta,x=x,ww=ww,eta=eta,type=type)$best_lambda
  cve = Betafunc(L=L,R=R,T=T,delta=delta,x=x,ww=ww,eta=eta,type=type)$cve
  }
