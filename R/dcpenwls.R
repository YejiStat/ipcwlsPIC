...
DCdata = function(n, case, beta, sigma){
  if(case=="norm75"){err=rnorm(n); l=8.7; r=28.5}
  if(case=="norm65"){err=rnorm(n); l=9.4; r=18.5}
  if(case=="gum75"){err=revd(n); l=4.5; r=14.5}
  if(case=="gum65"){err=revd(n); l=5.3; r=9.9}
  if(case=="t75"){err=rt(n, 7); l=8.3; r=26.5}
  if(case=="t65"){err=rt(n, 7); l=9.4; r=19.5}
  if(case=="gam75"){err=(rgamma(n,shape=1)); l=5.8; r=14.5}
  if(case=="gam65"){err=(rgamma(n,shape=1)); l=6.5; r=11.9}
  if(case=="exp75"){err=rexp(n,1); l=6.1; r=17.9}
  if(case=="exp65"){err=rexp(n,1); l=6.6; r=12.9}
  ...
  # T = 3*x1+1*x2+1*xi[,1]+0*xi[,2]+0*xi[,3]+0*xi[,4]+0*xi[,5]+1.5*xi[,6]+(err)*(1.3-0.1*x2)
  T = 3*x1+1*x2+1*xi[,1]+0*xi[,2]+0*xi[,3]+0*xi[,4]+0*xi[,5]+1.5*xi[,6]+(err)*sigma
  L=runif(n,-4.3,l); R=L+runif(n,4.1,r)
  Y=pmin(R,pmax(T,L))
  ...
  d=data.frame(Y=Y,L=L,R=R,T=T,delta=delta,x1=x1,x2=x2,xi)
  d[order(Y),]
}

Etable=function(type,x,est,se,beta0,AMADest,MRMEest,corest,incorest,tol=1e-2){
  p=ncol(x)
...
est=biasabs=se=AMADest=MRMEest=corest=incorest=NULL; nsim=1000; k=1
for (i in 1:nsim) {
  # library(doMC)
  # numcore = 1
  ...
  options(warn=-1);
  set.seed(i)
  d=DCdata(n=n,case = case[k], sigma=sigma)
  L=d$L; R=d$R; T=d$T; delta=d$delta; x=as.matrix(with(d,cbind(x1,x2,x3,x4,x5,x6,x7,x8)));
  setwd("~/Library/CloudStorage/OneDrive-고려대학교/KoreaGrad/Papers/IPCW_WLS/Codesim/230417/DCpen")
  if(wttype=="Beran"){
    hlimit=0.1
    fit=dcpenwls(L=L, R=R, T=T, delta=delta, x=x, hlimit=hlimit, wttype = wttype,type=type,beta0=c(3,1,1,0,0,0,0,1.5), nonzero.index = c(1,2,3,8))
  }else{
    fit=dcpenwls(L=L, R=R, T=T, delta=delta, x=x, wttype = wttype,type=type,beta0=c(3,1,1,0,0,0,0,1.5), nonzero.index = c(1,2,3,8))
  }
  ...
  esttab=Etable(type=type,x=x,est=est,se=se,AMADest=AMADest,MRMEest=MRMEest,corest=corest,incorest=incorest,beta0=c(3,1,1,0,0,0,0,1.5))
  MRMEest = rbind(MRMEest, esttab$mrme_beta)
  if(i>1 & (i%%5)==0) print( rbind( c(i,table(d$delta)/nrow(d),i,i,table(d$delta)/nrow(d),nsim-i), 
                                    Etable(type=type,x=x,est,se,AMADest,MRMEest,corest,incorest,beta0=c(3,1,1,0,0,0,0,1.5))[,-c(1)] ) )
  if(i>1 & (i%%5)==0) write.matrix( rbind( Etable(type=type,x=x,est,se,AMADest,MRMEest,corest,incorest,beta0=c(3,1,1,0,0,0,0,1.5))[,-c(1)] ),
                                    paste0("DCpen","_",nsim,"_",n,"_",case[k],"_",type,"_",wttype,"_",".txt"))
}
